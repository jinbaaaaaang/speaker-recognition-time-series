...
AI Hub 537번 데이터셋(common 카테고리)의 경량 샘플 데이터를 사용해 화자 50명의 음성을 분석합니다.
...
WAV 파일은 초당 16,000개의 숫자가 시간 순서로 나열된 시계열 데이터입니다. 순서를 바꾸면 말이 안 되는 소음이 되는 것처럼, 음성은 순서 자체가 의미를 갖습니다.

### 1. Lookback Window

과거 N초의 음성을 보고 화자를 예측하는 구조로, 시계열 분석의 lookback window 개념을 그대로 적용합니다. 이 프로젝트는 lookback 길이(0.5초 / 1.0초 / 1.5초 / 3.0초 / 전체)를 변수로 만들어 **"얼마나 들어야 누구인지 알 수 있는가"** 를 실험합니다.

### 2. Delta MFCC — 시간 변화량 반영

기본 MFCC만 쓰면 각 프레임의 음색만 잡고 시간 흐름은 잡지 못합니다. Delta MFCC(1차 변화량)와 Delta-Delta MFCC(2차 변화량)를 추가하면 목소리가 시간에 따라 어떻게 변하는지를 피처로 반영할 수 있습니다. 이것이 이 프로젝트를 단순 tabular 분류가 아닌 시계열 분석으로 만드는 핵심입니다.

### 3. 정보 누적 패턴

음성이 길어질수록 세 모델 모두에서 정확도가 꾸준히 올라가고, Cosine Similarity도 같은 화자끼리는 높아지고 다른 화자끼리는 낮아집니다. 이는 시계열 데이터의 핵심 특성인 **"정보가 시간에 따라 누적된다"** 는 것을 실험으로 증명합니다. 또한 이 패턴이 세 가지 서로 다른 모델에서 동일하게 나타났다는 것은, 특정 모델에 의존한 결과가 아니라 데이터 자체의 시계열적 특성에서 비롯된 것임을 의미합니다.
